...
for (let num = 1; num >= 1 && num <= 100; num ++) {
    if (num % 3 ==0 && num % 5 == 0) {
        console.log(`The number is ${num}, so fizz buzz on that`);
    } else if (num % 5 == 0) {
        console.log(`The number is ${num}, so buzz on that`);
    } else if (num % 3 == 0) {
        console.log(`The number is ${num}, so fizz on that`);
    } else {
        console.log(`The number is ${num}, so I can't fizz, buzz, or fizz buzz on that`);
    }
}
...
// pseudo code time!
// copy all characters in a string into a cell variable. 
// when you hit the comma, end the cell and move to the next
// keep repeating until you hit the end of the row (aka once you get to the new line escape characters)
// move to the next row and repeat the process - re-iterate through the characters until you hit a comma or a line ending
...
for (const c of string) {
    if (c = ",") {
        //then store into a variable... do I increment as cell1++??
    } else if ((c = "/n") | (c = "/r")) {
        //move to next line or start over as new cell1
    } else {
        c++
    }
}
